...
namespace TodoApi.Services
{
  ...
  public class ModelService : IModelService
  {
    ...
    public Data GetData(string model, string? where = null, List<string>? whereParams = null, List<string>? includes = null, int take = 0, int skip = 0, string? orderBy = null)
    {
      ...
      // This checks if Model is included in the includes property and throws an exception. I still don't know whether I should do this or not. 
      // if (includes != null)
      // {
      //   foreach (var include in includes)
      //   {
      //     if (include.Split('.').Where(n => n.ToLower() == typeDisplayNametype?.ToLower()).FirstOrDefault() != null)
      //     {
      //       throw new Exception("You shouldn't reference the same model");
      //     }
      //   }
      // }
      ...
      var set = ((IQueryable)setMethod.Invoke(_context, null)!);
      ...
      return new Data { data = set.ToDynamicList(), count = 0 };
    }
  }
}
...
// Some Bugs for now:
// Orderby doesn't work with navigation properties of type collection